...
export function renderPost(doc) {
  ...
  div.insertAdjacentHTML(
    ...
    <div class="timePost">${new Date(
      doc.date.seconds * 1000
    ).toLocaleDateString("es-CO")}</div>
    </div>
    ...
    <nav id="optionPost" class="optionPost">
        <ul>
            <li><button class="button_general" id="editPost-${
              doc.id
            }">Edit</button></li>
            <li><button class="button_general" id="deletePost-${
              doc.id
            }">Delete</button></li>
        </ul>
    </nav>
    ...
    <button class="btn-like" id="like-${
      doc.id
    }"><img class="default-like" src=./assets/like.svg></button>
    ...
    <div><img class="liked-${
      doc.id
    }" src=./assets/gleamLike.svg alt="liked"></div>
    </div>
    </div>
